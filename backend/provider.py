...
NOTES_PROMPT = '''아래 내용을 한국어 Obsidian 학습 노트로 편집해줘. 입력이 영어여도 한국어로 작성한다.
목표: 전사문을 불릿으로 옮기는 것이 아니라, 강의 흐름을 유지한 채 핵심과 부연의 관계가 한눈에 보이는 압축 필기.
문체의 필수 기준:
- 완결된 서술문을 기본으로 쓰지 않는다. 의미가 유지되면 조사·대명사·접속어·서술 어미를 생략하고 명사구·짧은 구문으로 쓴다.
- 예: '인공지능은 우리의 일상을 매개하며 취향과 세계관을 형성합니다.' → 'AI: 일상 매개, 취향·세계관 형성'.
- '우리는 ~할 수 있다', '~라는 것이다', '~하는 것이 중요하다', '이것은 ~을 의미한다' 같은 전사체를 반복하지 않는다. 부정·가능성·조건·행위 주체는 의미가 바뀌면 반드시 보존한다.
계층의 필수 기준:
- 중요하고 포괄적인 개념·주장·결론을 부모 불릿에 먼저 압축하고, 원문에 나온 근거·설명·예시를 그 아래에 묶는다. 문장 길이가 아니라 논리 관계로 깊이를 결정한다.
- 서로 다른 역할의 설명·사례를 같은 깊이에 길게 나열하지 않는다. 예시 자체에 해설이 있으면 그 해설은 예시 아래 한 단계 더 들여쓴다.
- 부모와 자식이 같은 뜻을 반복하면 합친다. 불릿 하나는 한 논점만 담고 짧게 쓴다. 짧은 문장 두세 개마다 새 제목을 만들지 않는다.
변환 예시(형식 참고용, 입력에 없는 내용은 사용 금지):
## 기술: 도구에서 환경으로
- 도구 → 환경: 인간 삶의 조건 자체를 형성
    - 기존 도구: 사용자가 도구 외부에서 통제
        - 망치: 못 박기 / 자동차: 이동 / 컴퓨터: 정보 처리
    - AI 환경: 선택·주의·세계관에 영향
        - SNS 알고리즘: 노출 정보·참여 대상 선별
            - 누적 효과: 취향·주의·세계관 형성
    - 핵심 질문: 기술을 사용하는가, 기술이 만든 조건 속에 사는가?
