...
try {
  ...
  $ready = $health.app -eq 'lecture-notes'
} catch {}
...
if (-not $ready) {
  ...
  for ($i = 0; $i -lt 30; $i++) {
    ...
    try {
      $health = Invoke-RestMethod 'http://127.0.0.1:18765/health' -TimeoutSec 1
      if ($health.app -eq 'lecture-notes') {
        $ready = $true
        break
      }
    } catch {}
...
if (-not $ready) {
  throw 'Could not start Lecture Notes. Check .local/server-error.log.'
}
